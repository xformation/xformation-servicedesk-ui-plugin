...
export class Multiselect extends React.Component<any, any> {
  ...
  removeSelectedValuesFromOptions(skipCheck: any) {
    ...
    if (isObject) {
      let optionList = unfilteredOptions.filter((item: any) => {
        return selectedValues.findIndex((v: any) => v[displayValue] === item[displayValue]) === -1 ? true : false;
      });
      if (groupBy) {
    ...
    let optionList = unfilteredOptions.filter((item: any) => selectedValues.indexOf(item) === -1);

    this.setState({ options: optionList, filteredOptions: optionList }, this.filterOptionsByInput);
  }

  groupByOptions(options: any) {
    const { groupBy } = this.props;
    const groupedObject = options.reduce(function (r: any, a: any) {
  ...
  filterOptionsByInput() {
    ...
    if (isObject) {
      options = filteredOptions.filter((i: any) => this.matchValues(i[displayValue], inputValue));
    } else {
      options = filteredOptions.filter((i: any) => this.matchValues(i, inputValue));
    }
    this.groupByOptions(options);
    this.setState({ options });
  }

  matchValues(value: any, search: any) {
  ...
  onRemoveSelectedItem(item: any) {
    let { selectedValues, index = 0, isObject } = this.state;
    const { onRemove, showCheckbox, displayValue } = this.props;
    if (isObject) {
      index = selectedValues.findIndex((i: any) => i[displayValue] === item[displayValue]);
    } else {
  ...
  isSelectedValue(item: any) {
    const { isObject, displayValue } = this.props;
    const { selectedValues } = this.state;
    if (isObject) {
      return selectedValues.filter((i: any) => i[displayValue] === item[displayValue]).length > 0;
    }
    return selectedValues.filter((i: any) => i === item).length > 0;
  }
    ...
    return Object.keys(groupedObject).map((obj) => {
      return (
        <React.Fragment key={obj}>
          ...
          {groupedObject[obj].map((option: any, i: any) => (
            <li
              key={`option${i}`}
              style={style['option']}
  ...
  renderNormalOption() {
    ...
    return this.state.options.map((option: any, i: any) => (
      <li
        key={`option${i}`}
        style={style['option']}
  ...
  renderSelectedList() {
    ...
    const { selectedValues } = this.state;
    return selectedValues.map((value: any, index: any) => (
      <span
  ...
  isDisablePreSelectedValues(value: any) {
    ...
    if (isObject) {
      return preSelectedValues.filter((i: any) => i[displayValue] === value[displayValue]).length > 0;
    }
    return preSelectedValues.filter((i: any) => i === value).length > 0;
  }

  fadeOutSelection(item: any) {
