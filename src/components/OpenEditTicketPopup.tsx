...
export class OpenEditTicketPopup extends React.Component<any, any> {
  ...
  fetchData = () => {
    RestService.getData(config.SERVICEDESK_API_URL + '/api/contacts', null, null).then((response: any) => {
  ...
  getTicketByID = async (id: any) => {
    let res;
    try {
      await RestService.getData(config.GET_TICKET_BY_ID + '/' + id, null, null).then((response: any) => {
  ...
  toggle = async (selectedTicket: any) => {
    ...
    try {
      await RestService.getData(config.GET_TICKET_BY_ID + '/' + selectedTicket.id, null, null).then((response: any) => {
        console.log('response', response);
